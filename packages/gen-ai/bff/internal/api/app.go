...
func (app *App) Routes() http.Handler {
	...
	// No RequireAccessToService: unlike Install (which calls the external MaaS service
	// via shared credentials), Delete only performs K8s API calls using the user's own
	// token, so RBAC is already enforced by the API server.
